...
async fn submit_simple() {
    ...
    let (port, server_task) = spawn_server(default_config("submit_simple")).await;
...
async fn submit_and_wait() {
    ...
    let (port, server_task) = spawn_server(default_config("submit_and_wait")).await;
    ...
    loop {
        ...
        if response.status() != reqwest::StatusCode::OK && response.status() != reqwest::StatusCode::NOT_FOUND {
